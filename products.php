<!DOCTYPE html>
<html lang="ru" dir="ltr">

<head>
  <meta charset="utf-8">
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
  <meta name="viewport" content="width=device-width, initial-scale=1, user-scalable=no">
  <title>Товары</title>
  <link href="https://fonts.googleapis.com/css2?family=Nunito:ital,wght@0,300;0,400;0,600;0,700;1,300;1,400;1,600;1,700&display=swap" rel="stylesheet">
  <link rel="shortcut icon" href="img/favicon.ico" type="image/x-icon">
  <link rel="icon" href="/favicon.ico" type="image/x-icon">
  <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css" integrity="sha384-Vkoo8x4CGsO3+Hhxv8T/Q5PaXtkKtu6ug5TOeNV6gBiFeWPGFN9MuhOf23Q9Ifjh" crossorigin="anonymous">
  <link rel="stylesheet" href="css/style.css">
</head>

<body>
  <?php include 'header.php' ?>
  <div class="content">
    <h2 class="title-2">
      Категория товаров
    </h2>
    <div class="container">
      <div class="filter">
        <div class="filter__text">Сортировать по:</div>
        <a href="#" class="filter__item js-filter-price">
          <span class="filter__item-text">Цене</span>
          <span class="filter__item-arrow" style="background-image: url(img/arrow.svg)"></span>
        </a>
        <a href="#" class="filter__item js-filter-age">
          <span class="filter__item-text">Имени</span>
          <span class="filter__item-arrow" style="background-image: url(img/arrow.svg)"></span>
        </a>
      </div>
      <div class="product-catalog">
        <a href="product.php" class="product-card" id="1">
          <object>
            <a class="izbr-button">
              <svg width="20" height="21" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                <g clip-path="url(#clip0)">
                  <path d="M14.5818 1C12.2909 1 10.7363 2.22727 9.99994 3.12727C9.26358 2.30909 7.70904 1 5.41813 1C2.30904 1 0.181763 3.94546 0.181763 7.21818C0.181763 9.67273 1.40904 14.4182 9.67267 18.9182C9.83631 19 10.0818 19 10.3272 18.9182C18.5909 14.4182 19.8181 9.67273 19.8181 7.21818C19.8181 3.86364 17.6909 1 14.5818 1ZM9.99994 17.6091C5.99085 15.4 1.49085 11.7182 1.49085 7.21818C1.49085 4.68182 2.96358 2.30909 5.41813 2.30909C7.21813 2.30909 8.77267 3.45455 9.42722 4.51818C9.67267 4.92727 10.2454 4.92727 10.5727 4.51818C10.5727 4.51818 11.9636 2.30909 14.5818 2.30909C17.0363 2.30909 18.509 4.68182 18.509 7.21818C18.509 11.7182 14.009 15.4 9.99994 17.6091Z" fill="#FB4B54" stroke="white" stroke-width="0.25" stroke-miterlimit="10"/>
                </g>
                <rect x="19.8182" y="13.6235" width="4.20779" height="9.81818" rx="2" transform="rotate(90 19.8182 13.6235)" fill="white"/>
                <rect x="12.8051" y="10.8182" width="4.20779" height="9.81818" rx="2" fill="white"/>
                <rect x="18.4156" y="15.0259" width="1.4026" height="7.01299" rx="0.701299" transform="rotate(90 18.4156 15.0259)" fill="#FB4B54"/>
                <rect x="14.2078" y="12.2206" width="1.4026" height="7.01299" rx="0.701299" fill="#FB4B54"/>
                <defs>
                  <clipPath id="clip0">
                    <rect width="19.6364" height="19.6364" fill="white" transform="translate(0.181763 0.181824)"/>
                  </clipPath>
                </defs>
              </svg>
            </a>
          </object>
          <object>
            <a data-toggle="modal" data-target="#button_tovar" class="product-card__search d-flex">
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M13.3617 12.7518L10.017 9.28631C10.8769 8.26788 11.3481 6.98648 11.3481 5.65249C11.3481 2.53576 8.8027 0 5.67407 0C2.54544 0 0 2.53576 0 5.65249C0 8.76923 2.54544 11.305 5.67407 11.305C6.8486 11.305 7.96787 10.9521 8.92482 10.2821L12.295 13.7739C12.4358 13.9196 12.6253 14 12.8283 14C13.0205 14 13.2028 13.927 13.3412 13.7943C13.6353 13.5124 13.6447 13.045 13.3617 12.7518ZM5.67407 1.47456C7.98662 1.47456 9.86795 3.34873 9.86795 5.65249C9.86795 7.95625 7.98662 9.83043 5.67407 9.83043C3.36152 9.83043 1.48019 7.95625 1.48019 5.65249C1.48019 3.34873 3.36152 1.47456 5.67407 1.47456Z" fill="#FB4B54"/>
              </svg>
            </a>
          </object>
          <div class="product-card__img">
            <img src="img/c3po.png" alt="">
          </div>
          <div class="product-card__content">
            <div class="product-card__name">
              C-3PO
            </div>
            <div class="product-card__info">
              <div class="product-price">
                <div class="product-price__old">
                  15000 $
                </div>
                <div class="product-price__actual">
                  <div class="product-price__new">
                    14000 $
                  </div>
                  <div class="product-price__quantity">
                    за 1 шт
                  </div>
                </div>
              </div>
              <object>
                <a data-toggle="modal" data-target="#button-tovar-v-korzine" class="in-cart">
                  В корзину
                </a>
              </object>
            </div>
          </div>
        </a>
        <a href="product.php" class="product-card" id="2">
          <object>
            <a class="izbr-button">
              <svg width="20" height="21" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                <g clip-path="url(#clip0)">
                  <path d="M14.5818 1C12.2909 1 10.7363 2.22727 9.99994 3.12727C9.26358 2.30909 7.70904 1 5.41813 1C2.30904 1 0.181763 3.94546 0.181763 7.21818C0.181763 9.67273 1.40904 14.4182 9.67267 18.9182C9.83631 19 10.0818 19 10.3272 18.9182C18.5909 14.4182 19.8181 9.67273 19.8181 7.21818C19.8181 3.86364 17.6909 1 14.5818 1ZM9.99994 17.6091C5.99085 15.4 1.49085 11.7182 1.49085 7.21818C1.49085 4.68182 2.96358 2.30909 5.41813 2.30909C7.21813 2.30909 8.77267 3.45455 9.42722 4.51818C9.67267 4.92727 10.2454 4.92727 10.5727 4.51818C10.5727 4.51818 11.9636 2.30909 14.5818 2.30909C17.0363 2.30909 18.509 4.68182 18.509 7.21818C18.509 11.7182 14.009 15.4 9.99994 17.6091Z" fill="#FB4B54" stroke="white" stroke-width="0.25" stroke-miterlimit="10"/>
                </g>
                <rect x="19.8182" y="13.6235" width="4.20779" height="9.81818" rx="2" transform="rotate(90 19.8182 13.6235)" fill="white"/>
                <rect x="12.8051" y="10.8182" width="4.20779" height="9.81818" rx="2" fill="white"/>
                <rect x="18.4156" y="15.0259" width="1.4026" height="7.01299" rx="0.701299" transform="rotate(90 18.4156 15.0259)" fill="#FB4B54"/>
                <rect x="14.2078" y="12.2206" width="1.4026" height="7.01299" rx="0.701299" fill="#FB4B54"/>
                <defs>
                  <clipPath id="clip0">
                    <rect width="19.6364" height="19.6364" fill="white" transform="translate(0.181763 0.181824)"/>
                  </clipPath>
                </defs>
              </svg>
            </a>
          </object>
          <object>
            <a data-toggle="modal" data-target="#button_tovar" class="product-card__search d-flex">
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M13.3617 12.7518L10.017 9.28631C10.8769 8.26788 11.3481 6.98648 11.3481 5.65249C11.3481 2.53576 8.8027 0 5.67407 0C2.54544 0 0 2.53576 0 5.65249C0 8.76923 2.54544 11.305 5.67407 11.305C6.8486 11.305 7.96787 10.9521 8.92482 10.2821L12.295 13.7739C12.4358 13.9196 12.6253 14 12.8283 14C13.0205 14 13.2028 13.927 13.3412 13.7943C13.6353 13.5124 13.6447 13.045 13.3617 12.7518ZM5.67407 1.47456C7.98662 1.47456 9.86795 3.34873 9.86795 5.65249C9.86795 7.95625 7.98662 9.83043 5.67407 9.83043C3.36152 9.83043 1.48019 7.95625 1.48019 5.65249C1.48019 3.34873 3.36152 1.47456 5.67407 1.47456Z" fill="#FB4B54"/>
              </svg>
            </a>
          </object>
          <div class="product-card__img">
            <img src="img/r2d2.jpg" alt="">
          </div>
          <div class="product-card__content">
            <div class="product-card__name">
              R2-D2
            </div>
            <div class="product-card__info">
              <div class="product-price">
                <div class="product-price__old">
                  18000 $
                </div>
                <div class="product-price__actual">
                  <div class="product-price__new">
                    15000 $
                  </div>
                  <div class="product-price__quantity">
                    за 1 шт
                  </div>
                </div>
              </div>
              <object>
                <a data-toggle="modal" data-target="#button-tovar-v-korzine" class="in-cart">
                  В корзину
                </a>
              </object>
            </div>
          </div>
        </a>
        <a href="product.php" class="product-card" id="3">
          <object>
            <a class="izbr-button">
              <svg width="20" height="21" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                <g clip-path="url(#clip0)">
                  <path d="M14.5818 1C12.2909 1 10.7363 2.22727 9.99994 3.12727C9.26358 2.30909 7.70904 1 5.41813 1C2.30904 1 0.181763 3.94546 0.181763 7.21818C0.181763 9.67273 1.40904 14.4182 9.67267 18.9182C9.83631 19 10.0818 19 10.3272 18.9182C18.5909 14.4182 19.8181 9.67273 19.8181 7.21818C19.8181 3.86364 17.6909 1 14.5818 1ZM9.99994 17.6091C5.99085 15.4 1.49085 11.7182 1.49085 7.21818C1.49085 4.68182 2.96358 2.30909 5.41813 2.30909C7.21813 2.30909 8.77267 3.45455 9.42722 4.51818C9.67267 4.92727 10.2454 4.92727 10.5727 4.51818C10.5727 4.51818 11.9636 2.30909 14.5818 2.30909C17.0363 2.30909 18.509 4.68182 18.509 7.21818C18.509 11.7182 14.009 15.4 9.99994 17.6091Z" fill="#FB4B54" stroke="white" stroke-width="0.25" stroke-miterlimit="10"/>
                </g>
                <rect x="19.8182" y="13.6235" width="4.20779" height="9.81818" rx="2" transform="rotate(90 19.8182 13.6235)" fill="white"/>
                <rect x="12.8051" y="10.8182" width="4.20779" height="9.81818" rx="2" fill="white"/>
                <rect x="18.4156" y="15.0259" width="1.4026" height="7.01299" rx="0.701299" transform="rotate(90 18.4156 15.0259)" fill="#FB4B54"/>
                <rect x="14.2078" y="12.2206" width="1.4026" height="7.01299" rx="0.701299" fill="#FB4B54"/>
                <defs>
                  <clipPath id="clip0">
                    <rect width="19.6364" height="19.6364" fill="white" transform="translate(0.181763 0.181824)"/>
                  </clipPath>
                </defs>
              </svg>
            </a>
          </object>
          <object>
            <a data-toggle="modal" data-target="#button_tovar" class="product-card__search d-flex">
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M13.3617 12.7518L10.017 9.28631C10.8769 8.26788 11.3481 6.98648 11.3481 5.65249C11.3481 2.53576 8.8027 0 5.67407 0C2.54544 0 0 2.53576 0 5.65249C0 8.76923 2.54544 11.305 5.67407 11.305C6.8486 11.305 7.96787 10.9521 8.92482 10.2821L12.295 13.7739C12.4358 13.9196 12.6253 14 12.8283 14C13.0205 14 13.2028 13.927 13.3412 13.7943C13.6353 13.5124 13.6447 13.045 13.3617 12.7518ZM5.67407 1.47456C7.98662 1.47456 9.86795 3.34873 9.86795 5.65249C9.86795 7.95625 7.98662 9.83043 5.67407 9.83043C3.36152 9.83043 1.48019 7.95625 1.48019 5.65249C1.48019 3.34873 3.36152 1.47456 5.67407 1.47456Z" fill="#FB4B54"/>
              </svg>
            </a>
          </object>
          <div class="product-card__img">
            <img src="img/r4.jpg" alt="">
          </div>
          <div class="product-card__content">
            <div class="product-card__name">
              R4-P17
            </div>
            <div class="product-card__info">
              <div class="product-price">
                <div class="product-price__old">
                  10000 $
                </div>
                <div class="product-price__actual">
                  <div class="product-price__new">
                    4000 $
                  </div>
                  <div class="product-price__quantity">
                    за 1 шт
                  </div>
                </div>
              </div>
              <object>
                <a data-toggle="modal" data-target="#button-tovar-v-korzine" class="in-cart">
                  В корзину
                </a>
              </object>
            </div>
          </div>
        </a>
        <a href="product.php" class="product-card" id="4">
          <object>
            <a class="izbr-button">
              <svg width="20" height="21" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                <g clip-path="url(#clip0)">
                  <path d="M14.5818 1C12.2909 1 10.7363 2.22727 9.99994 3.12727C9.26358 2.30909 7.70904 1 5.41813 1C2.30904 1 0.181763 3.94546 0.181763 7.21818C0.181763 9.67273 1.40904 14.4182 9.67267 18.9182C9.83631 19 10.0818 19 10.3272 18.9182C18.5909 14.4182 19.8181 9.67273 19.8181 7.21818C19.8181 3.86364 17.6909 1 14.5818 1ZM9.99994 17.6091C5.99085 15.4 1.49085 11.7182 1.49085 7.21818C1.49085 4.68182 2.96358 2.30909 5.41813 2.30909C7.21813 2.30909 8.77267 3.45455 9.42722 4.51818C9.67267 4.92727 10.2454 4.92727 10.5727 4.51818C10.5727 4.51818 11.9636 2.30909 14.5818 2.30909C17.0363 2.30909 18.509 4.68182 18.509 7.21818C18.509 11.7182 14.009 15.4 9.99994 17.6091Z" fill="#FB4B54" stroke="white" stroke-width="0.25" stroke-miterlimit="10"/>
                </g>
                <rect x="19.8182" y="13.6235" width="4.20779" height="9.81818" rx="2" transform="rotate(90 19.8182 13.6235)" fill="white"/>
                <rect x="12.8051" y="10.8182" width="4.20779" height="9.81818" rx="2" fill="white"/>
                <rect x="18.4156" y="15.0259" width="1.4026" height="7.01299" rx="0.701299" transform="rotate(90 18.4156 15.0259)" fill="#FB4B54"/>
                <rect x="14.2078" y="12.2206" width="1.4026" height="7.01299" rx="0.701299" fill="#FB4B54"/>
                <defs>
                  <clipPath id="clip0">
                    <rect width="19.6364" height="19.6364" fill="white" transform="translate(0.181763 0.181824)"/>
                  </clipPath>
                </defs>
              </svg>
            </a>
          </object>
          <object>
            <a data-toggle="modal" data-target="#button_tovar" class="product-card__search d-flex">
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M13.3617 12.7518L10.017 9.28631C10.8769 8.26788 11.3481 6.98648 11.3481 5.65249C11.3481 2.53576 8.8027 0 5.67407 0C2.54544 0 0 2.53576 0 5.65249C0 8.76923 2.54544 11.305 5.67407 11.305C6.8486 11.305 7.96787 10.9521 8.92482 10.2821L12.295 13.7739C12.4358 13.9196 12.6253 14 12.8283 14C13.0205 14 13.2028 13.927 13.3412 13.7943C13.6353 13.5124 13.6447 13.045 13.3617 12.7518ZM5.67407 1.47456C7.98662 1.47456 9.86795 3.34873 9.86795 5.65249C9.86795 7.95625 7.98662 9.83043 5.67407 9.83043C3.36152 9.83043 1.48019 7.95625 1.48019 5.65249C1.48019 3.34873 3.36152 1.47456 5.67407 1.47456Z" fill="#FB4B54"/>
              </svg>
            </a>
          </object>
          <div class="product-card__img">
            <img src="img/chopper.jpg" alt="">
          </div>
          <div class="product-card__content">
            <div class="product-card__name">
              C1-10P
            </div>
            <div class="product-card__info">
              <div class="product-price">
                <div class="product-price__old">
                  9000 $
                </div>
                <div class="product-price__actual">
                  <div class="product-price__new">
                    4000 $
                  </div>
                  <div class="product-price__quantity">
                    за 1 шт
                  </div>
                </div>
              </div>
              <object>
                <a data-toggle="modal" data-target="#button-tovar-v-korzine" class="in-cart">
                  В корзину
                </a>
              </object>
            </div>
          </div>
        </a>
        <a href="product.php" class="product-card" id="5">
          <object>
            <a class="izbr-button">
              <svg width="20" height="21" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                <g clip-path="url(#clip0)">
                  <path d="M14.5818 1C12.2909 1 10.7363 2.22727 9.99994 3.12727C9.26358 2.30909 7.70904 1 5.41813 1C2.30904 1 0.181763 3.94546 0.181763 7.21818C0.181763 9.67273 1.40904 14.4182 9.67267 18.9182C9.83631 19 10.0818 19 10.3272 18.9182C18.5909 14.4182 19.8181 9.67273 19.8181 7.21818C19.8181 3.86364 17.6909 1 14.5818 1ZM9.99994 17.6091C5.99085 15.4 1.49085 11.7182 1.49085 7.21818C1.49085 4.68182 2.96358 2.30909 5.41813 2.30909C7.21813 2.30909 8.77267 3.45455 9.42722 4.51818C9.67267 4.92727 10.2454 4.92727 10.5727 4.51818C10.5727 4.51818 11.9636 2.30909 14.5818 2.30909C17.0363 2.30909 18.509 4.68182 18.509 7.21818C18.509 11.7182 14.009 15.4 9.99994 17.6091Z" fill="#FB4B54" stroke="white" stroke-width="0.25" stroke-miterlimit="10"/>
                </g>
                <rect x="19.8182" y="13.6235" width="4.20779" height="9.81818" rx="2" transform="rotate(90 19.8182 13.6235)" fill="white"/>
                <rect x="12.8051" y="10.8182" width="4.20779" height="9.81818" rx="2" fill="white"/>
                <rect x="18.4156" y="15.0259" width="1.4026" height="7.01299" rx="0.701299" transform="rotate(90 18.4156 15.0259)" fill="#FB4B54"/>
                <rect x="14.2078" y="12.2206" width="1.4026" height="7.01299" rx="0.701299" fill="#FB4B54"/>
                <defs>
                  <clipPath id="clip0">
                    <rect width="19.6364" height="19.6364" fill="white" transform="translate(0.181763 0.181824)"/>
                  </clipPath>
                </defs>
              </svg>
            </a>
          </object>
          <object>
            <a data-toggle="modal" data-target="#button_tovar" class="product-card__search d-flex">
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M13.3617 12.7518L10.017 9.28631C10.8769 8.26788 11.3481 6.98648 11.3481 5.65249C11.3481 2.53576 8.8027 0 5.67407 0C2.54544 0 0 2.53576 0 5.65249C0 8.76923 2.54544 11.305 5.67407 11.305C6.8486 11.305 7.96787 10.9521 8.92482 10.2821L12.295 13.7739C12.4358 13.9196 12.6253 14 12.8283 14C13.0205 14 13.2028 13.927 13.3412 13.7943C13.6353 13.5124 13.6447 13.045 13.3617 12.7518ZM5.67407 1.47456C7.98662 1.47456 9.86795 3.34873 9.86795 5.65249C9.86795 7.95625 7.98662 9.83043 5.67407 9.83043C3.36152 9.83043 1.48019 7.95625 1.48019 5.65249C1.48019 3.34873 3.36152 1.47456 5.67407 1.47456Z" fill="#FB4B54"/>
              </svg>
            </a>
          </object>
          <div class="product-card__img">
            <img src="img/b1.jpg" alt="">
          </div>
          <div class="product-card__content">
            <div class="product-card__name">
              B1
            </div>
            <div class="product-card__info">
              <div class="product-price">
                <div class="product-price__old">
                  15000 $
                </div>
                <div class="product-price__actual">
                  <div class="product-price__new">
                    4000 $
                  </div>
                  <div class="product-price__quantity">
                    за 10 шт
                  </div>
                </div>
              </div>
              <object>
                <a data-toggle="modal" data-target="#button-tovar-v-korzine" class="in-cart">
                  В корзину
                </a>
              </object>
            </div>
          </div>
        </a>
        <a href="product.php" class="product-card" id="6">
          <object>
            <a class="izbr-button">
              <svg width="20" height="21" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                <g clip-path="url(#clip0)">
                  <path d="M14.5818 1C12.2909 1 10.7363 2.22727 9.99994 3.12727C9.26358 2.30909 7.70904 1 5.41813 1C2.30904 1 0.181763 3.94546 0.181763 7.21818C0.181763 9.67273 1.40904 14.4182 9.67267 18.9182C9.83631 19 10.0818 19 10.3272 18.9182C18.5909 14.4182 19.8181 9.67273 19.8181 7.21818C19.8181 3.86364 17.6909 1 14.5818 1ZM9.99994 17.6091C5.99085 15.4 1.49085 11.7182 1.49085 7.21818C1.49085 4.68182 2.96358 2.30909 5.41813 2.30909C7.21813 2.30909 8.77267 3.45455 9.42722 4.51818C9.67267 4.92727 10.2454 4.92727 10.5727 4.51818C10.5727 4.51818 11.9636 2.30909 14.5818 2.30909C17.0363 2.30909 18.509 4.68182 18.509 7.21818C18.509 11.7182 14.009 15.4 9.99994 17.6091Z" fill="#FB4B54" stroke="white" stroke-width="0.25" stroke-miterlimit="10"/>
                </g>
                <rect x="19.8182" y="13.6235" width="4.20779" height="9.81818" rx="2" transform="rotate(90 19.8182 13.6235)" fill="white"/>
                <rect x="12.8051" y="10.8182" width="4.20779" height="9.81818" rx="2" fill="white"/>
                <rect x="18.4156" y="15.0259" width="1.4026" height="7.01299" rx="0.701299" transform="rotate(90 18.4156 15.0259)" fill="#FB4B54"/>
                <rect x="14.2078" y="12.2206" width="1.4026" height="7.01299" rx="0.701299" fill="#FB4B54"/>
                <defs>
                  <clipPath id="clip0">
                    <rect width="19.6364" height="19.6364" fill="white" transform="translate(0.181763 0.181824)"/>
                  </clipPath>
                </defs>
              </svg>
            </a>
          </object>
          <object>
            <a data-toggle="modal" data-target="#button_tovar" class="product-card__search d-flex">
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M13.3617 12.7518L10.017 9.28631C10.8769 8.26788 11.3481 6.98648 11.3481 5.65249C11.3481 2.53576 8.8027 0 5.67407 0C2.54544 0 0 2.53576 0 5.65249C0 8.76923 2.54544 11.305 5.67407 11.305C6.8486 11.305 7.96787 10.9521 8.92482 10.2821L12.295 13.7739C12.4358 13.9196 12.6253 14 12.8283 14C13.0205 14 13.2028 13.927 13.3412 13.7943C13.6353 13.5124 13.6447 13.045 13.3617 12.7518ZM5.67407 1.47456C7.98662 1.47456 9.86795 3.34873 9.86795 5.65249C9.86795 7.95625 7.98662 9.83043 5.67407 9.83043C3.36152 9.83043 1.48019 7.95625 1.48019 5.65249C1.48019 3.34873 3.36152 1.47456 5.67407 1.47456Z" fill="#FB4B54"/>
              </svg>
            </a>
          </object>
          <div class="product-card__img">
            <img src="img/c3po.png" alt="">
          </div>
          <div class="product-card__content">
            <div class="product-card__name">
              C-3PO
            </div>
            <div class="product-card__info">
              <div class="product-price">
                <div class="product-price__old">
                  15000 $
                </div>
                <div class="product-price__actual">
                  <div class="product-price__new">
                    14000 $
                  </div>
                  <div class="product-price__quantity">
                    за 1 шт
                  </div>
                </div>
              </div>
              <object>
                <a data-toggle="modal" data-target="#button-tovar-v-korzine" class="in-cart">
                  В корзину
                </a>
              </object>
            </div>
          </div>
        </a>
        <a href="product.php" class="product-card" id="7">
          <object>
            <a class="izbr-button">
              <svg width="20" height="21" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                <g clip-path="url(#clip0)">
                  <path d="M14.5818 1C12.2909 1 10.7363 2.22727 9.99994 3.12727C9.26358 2.30909 7.70904 1 5.41813 1C2.30904 1 0.181763 3.94546 0.181763 7.21818C0.181763 9.67273 1.40904 14.4182 9.67267 18.9182C9.83631 19 10.0818 19 10.3272 18.9182C18.5909 14.4182 19.8181 9.67273 19.8181 7.21818C19.8181 3.86364 17.6909 1 14.5818 1ZM9.99994 17.6091C5.99085 15.4 1.49085 11.7182 1.49085 7.21818C1.49085 4.68182 2.96358 2.30909 5.41813 2.30909C7.21813 2.30909 8.77267 3.45455 9.42722 4.51818C9.67267 4.92727 10.2454 4.92727 10.5727 4.51818C10.5727 4.51818 11.9636 2.30909 14.5818 2.30909C17.0363 2.30909 18.509 4.68182 18.509 7.21818C18.509 11.7182 14.009 15.4 9.99994 17.6091Z" fill="#FB4B54" stroke="white" stroke-width="0.25" stroke-miterlimit="10"/>
                </g>
                <rect x="19.8182" y="13.6235" width="4.20779" height="9.81818" rx="2" transform="rotate(90 19.8182 13.6235)" fill="white"/>
                <rect x="12.8051" y="10.8182" width="4.20779" height="9.81818" rx="2" fill="white"/>
                <rect x="18.4156" y="15.0259" width="1.4026" height="7.01299" rx="0.701299" transform="rotate(90 18.4156 15.0259)" fill="#FB4B54"/>
                <rect x="14.2078" y="12.2206" width="1.4026" height="7.01299" rx="0.701299" fill="#FB4B54"/>
                <defs>
                  <clipPath id="clip0">
                    <rect width="19.6364" height="19.6364" fill="white" transform="translate(0.181763 0.181824)"/>
                  </clipPath>
                </defs>
              </svg>
            </a>
          </object>
          <object>
            <a data-toggle="modal" data-target="#button_tovar" class="product-card__search d-flex">
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M13.3617 12.7518L10.017 9.28631C10.8769 8.26788 11.3481 6.98648 11.3481 5.65249C11.3481 2.53576 8.8027 0 5.67407 0C2.54544 0 0 2.53576 0 5.65249C0 8.76923 2.54544 11.305 5.67407 11.305C6.8486 11.305 7.96787 10.9521 8.92482 10.2821L12.295 13.7739C12.4358 13.9196 12.6253 14 12.8283 14C13.0205 14 13.2028 13.927 13.3412 13.7943C13.6353 13.5124 13.6447 13.045 13.3617 12.7518ZM5.67407 1.47456C7.98662 1.47456 9.86795 3.34873 9.86795 5.65249C9.86795 7.95625 7.98662 9.83043 5.67407 9.83043C3.36152 9.83043 1.48019 7.95625 1.48019 5.65249C1.48019 3.34873 3.36152 1.47456 5.67407 1.47456Z" fill="#FB4B54"/>
              </svg>
            </a>
          </object>
          <div class="product-card__img">
            <img src="img/r2d2.jpg" alt="">
          </div>
          <div class="product-card__content">
            <div class="product-card__name">
              R2-D2
            </div>
            <div class="product-card__info">
              <div class="product-price">
                <div class="product-price__old">
                  18000 $
                </div>
                <div class="product-price__actual">
                  <div class="product-price__new">
                    15000 $
                  </div>
                  <div class="product-price__quantity">
                    за 1 шт
                  </div>
                </div>
              </div>
              <object>
                <a data-toggle="modal" data-target="#button-tovar-v-korzine" class="in-cart">
                  В корзину
                </a>
              </object>
            </div>
          </div>
        </a>
        <a href="product.php" class="product-card" id="8">
          <object>
            <a class="izbr-button">
              <svg width="20" height="21" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                <g clip-path="url(#clip0)">
                  <path d="M14.5818 1C12.2909 1 10.7363 2.22727 9.99994 3.12727C9.26358 2.30909 7.70904 1 5.41813 1C2.30904 1 0.181763 3.94546 0.181763 7.21818C0.181763 9.67273 1.40904 14.4182 9.67267 18.9182C9.83631 19 10.0818 19 10.3272 18.9182C18.5909 14.4182 19.8181 9.67273 19.8181 7.21818C19.8181 3.86364 17.6909 1 14.5818 1ZM9.99994 17.6091C5.99085 15.4 1.49085 11.7182 1.49085 7.21818C1.49085 4.68182 2.96358 2.30909 5.41813 2.30909C7.21813 2.30909 8.77267 3.45455 9.42722 4.51818C9.67267 4.92727 10.2454 4.92727 10.5727 4.51818C10.5727 4.51818 11.9636 2.30909 14.5818 2.30909C17.0363 2.30909 18.509 4.68182 18.509 7.21818C18.509 11.7182 14.009 15.4 9.99994 17.6091Z" fill="#FB4B54" stroke="white" stroke-width="0.25" stroke-miterlimit="10"/>
                </g>
                <rect x="19.8182" y="13.6235" width="4.20779" height="9.81818" rx="2" transform="rotate(90 19.8182 13.6235)" fill="white"/>
                <rect x="12.8051" y="10.8182" width="4.20779" height="9.81818" rx="2" fill="white"/>
                <rect x="18.4156" y="15.0259" width="1.4026" height="7.01299" rx="0.701299" transform="rotate(90 18.4156 15.0259)" fill="#FB4B54"/>
                <rect x="14.2078" y="12.2206" width="1.4026" height="7.01299" rx="0.701299" fill="#FB4B54"/>
                <defs>
                  <clipPath id="clip0">
                    <rect width="19.6364" height="19.6364" fill="white" transform="translate(0.181763 0.181824)"/>
                  </clipPath>
                </defs>
              </svg>
            </a>
          </object>
          <object>
            <a data-toggle="modal" data-target="#button_tovar" class="product-card__search d-flex">
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M13.3617 12.7518L10.017 9.28631C10.8769 8.26788 11.3481 6.98648 11.3481 5.65249C11.3481 2.53576 8.8027 0 5.67407 0C2.54544 0 0 2.53576 0 5.65249C0 8.76923 2.54544 11.305 5.67407 11.305C6.8486 11.305 7.96787 10.9521 8.92482 10.2821L12.295 13.7739C12.4358 13.9196 12.6253 14 12.8283 14C13.0205 14 13.2028 13.927 13.3412 13.7943C13.6353 13.5124 13.6447 13.045 13.3617 12.7518ZM5.67407 1.47456C7.98662 1.47456 9.86795 3.34873 9.86795 5.65249C9.86795 7.95625 7.98662 9.83043 5.67407 9.83043C3.36152 9.83043 1.48019 7.95625 1.48019 5.65249C1.48019 3.34873 3.36152 1.47456 5.67407 1.47456Z" fill="#FB4B54"/>
              </svg>
            </a>
          </object>
          <div class="product-card__img">
            <img src="img/r4.jpg" alt="">
          </div>
          <div class="product-card__content">
            <div class="product-card__name">
              R4-P17
            </div>
            <div class="product-card__info">
              <div class="product-price">
                <div class="product-price__old">
                  30000 $
                </div>
                <div class="product-price__actual">
                  <div class="product-price__new">
                    24000 $
                  </div>
                  <div class="product-price__quantity">
                    за 1 шт
                  </div>
                </div>
              </div>
              <object>
                <a data-toggle="modal" data-target="#button-tovar-v-korzine" class="in-cart">
                  В корзину
                </a>
              </object>
            </div>
          </div>
        </a>
        <a href="product.php" class="product-card" id="9">
          <object>
            <a class="izbr-button">
              <svg width="20" height="21" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                <g clip-path="url(#clip0)">
                  <path d="M14.5818 1C12.2909 1 10.7363 2.22727 9.99994 3.12727C9.26358 2.30909 7.70904 1 5.41813 1C2.30904 1 0.181763 3.94546 0.181763 7.21818C0.181763 9.67273 1.40904 14.4182 9.67267 18.9182C9.83631 19 10.0818 19 10.3272 18.9182C18.5909 14.4182 19.8181 9.67273 19.8181 7.21818C19.8181 3.86364 17.6909 1 14.5818 1ZM9.99994 17.6091C5.99085 15.4 1.49085 11.7182 1.49085 7.21818C1.49085 4.68182 2.96358 2.30909 5.41813 2.30909C7.21813 2.30909 8.77267 3.45455 9.42722 4.51818C9.67267 4.92727 10.2454 4.92727 10.5727 4.51818C10.5727 4.51818 11.9636 2.30909 14.5818 2.30909C17.0363 2.30909 18.509 4.68182 18.509 7.21818C18.509 11.7182 14.009 15.4 9.99994 17.6091Z" fill="#FB4B54" stroke="white" stroke-width="0.25" stroke-miterlimit="10"/>
                </g>
                <rect x="19.8182" y="13.6235" width="4.20779" height="9.81818" rx="2" transform="rotate(90 19.8182 13.6235)" fill="white"/>
                <rect x="12.8051" y="10.8182" width="4.20779" height="9.81818" rx="2" fill="white"/>
                <rect x="18.4156" y="15.0259" width="1.4026" height="7.01299" rx="0.701299" transform="rotate(90 18.4156 15.0259)" fill="#FB4B54"/>
                <rect x="14.2078" y="12.2206" width="1.4026" height="7.01299" rx="0.701299" fill="#FB4B54"/>
                <defs>
                  <clipPath id="clip0">
                    <rect width="19.6364" height="19.6364" fill="white" transform="translate(0.181763 0.181824)"/>
                  </clipPath>
                </defs>
              </svg>
            </a>
          </object>
          <object>
            <a data-toggle="modal" data-target="#button_tovar" class="product-card__search d-flex">
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M13.3617 12.7518L10.017 9.28631C10.8769 8.26788 11.3481 6.98648 11.3481 5.65249C11.3481 2.53576 8.8027 0 5.67407 0C2.54544 0 0 2.53576 0 5.65249C0 8.76923 2.54544 11.305 5.67407 11.305C6.8486 11.305 7.96787 10.9521 8.92482 10.2821L12.295 13.7739C12.4358 13.9196 12.6253 14 12.8283 14C13.0205 14 13.2028 13.927 13.3412 13.7943C13.6353 13.5124 13.6447 13.045 13.3617 12.7518ZM5.67407 1.47456C7.98662 1.47456 9.86795 3.34873 9.86795 5.65249C9.86795 7.95625 7.98662 9.83043 5.67407 9.83043C3.36152 9.83043 1.48019 7.95625 1.48019 5.65249C1.48019 3.34873 3.36152 1.47456 5.67407 1.47456Z" fill="#FB4B54"/>
              </svg>
            </a>
          </object>
          <div class="product-card__img">
            <img src="img/chopper.jpg" alt="">
          </div>
          <div class="product-card__content">
            <div class="product-card__name">
              C1-10P
            </div>
            <div class="product-card__info">
              <div class="product-price">
                <div class="product-price__old">
                  29000 $
                </div>
                <div class="product-price__actual">
                  <div class="product-price__new">
                    24000 $
                  </div>
                  <div class="product-price__quantity">
                    за 1 шт
                  </div>
                </div>
              </div>
              <object>
                <a data-toggle="modal" data-target="#button-tovar-v-korzine" class="in-cart">
                  В корзину
                </a>
              </object>
            </div>
          </div>
        </a>
        <a href="product.php" class="product-card" id="10">
          <object>
            <a class="izbr-button">
              <svg width="20" height="21" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                <g clip-path="url(#clip0)">
                  <path d="M14.5818 1C12.2909 1 10.7363 2.22727 9.99994 3.12727C9.26358 2.30909 7.70904 1 5.41813 1C2.30904 1 0.181763 3.94546 0.181763 7.21818C0.181763 9.67273 1.40904 14.4182 9.67267 18.9182C9.83631 19 10.0818 19 10.3272 18.9182C18.5909 14.4182 19.8181 9.67273 19.8181 7.21818C19.8181 3.86364 17.6909 1 14.5818 1ZM9.99994 17.6091C5.99085 15.4 1.49085 11.7182 1.49085 7.21818C1.49085 4.68182 2.96358 2.30909 5.41813 2.30909C7.21813 2.30909 8.77267 3.45455 9.42722 4.51818C9.67267 4.92727 10.2454 4.92727 10.5727 4.51818C10.5727 4.51818 11.9636 2.30909 14.5818 2.30909C17.0363 2.30909 18.509 4.68182 18.509 7.21818C18.509 11.7182 14.009 15.4 9.99994 17.6091Z" fill="#FB4B54" stroke="white" stroke-width="0.25" stroke-miterlimit="10"/>
                </g>
                <rect x="19.8182" y="13.6235" width="4.20779" height="9.81818" rx="2" transform="rotate(90 19.8182 13.6235)" fill="white"/>
                <rect x="12.8051" y="10.8182" width="4.20779" height="9.81818" rx="2" fill="white"/>
                <rect x="18.4156" y="15.0259" width="1.4026" height="7.01299" rx="0.701299" transform="rotate(90 18.4156 15.0259)" fill="#FB4B54"/>
                <rect x="14.2078" y="12.2206" width="1.4026" height="7.01299" rx="0.701299" fill="#FB4B54"/>
                <defs>
                  <clipPath id="clip0">
                    <rect width="19.6364" height="19.6364" fill="white" transform="translate(0.181763 0.181824)"/>
                  </clipPath>
                </defs>
              </svg>
            </a>
          </object>
          <object>
            <a data-toggle="modal" data-target="#button_tovar" class="product-card__search d-flex">
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M13.3617 12.7518L10.017 9.28631C10.8769 8.26788 11.3481 6.98648 11.3481 5.65249C11.3481 2.53576 8.8027 0 5.67407 0C2.54544 0 0 2.53576 0 5.65249C0 8.76923 2.54544 11.305 5.67407 11.305C6.8486 11.305 7.96787 10.9521 8.92482 10.2821L12.295 13.7739C12.4358 13.9196 12.6253 14 12.8283 14C13.0205 14 13.2028 13.927 13.3412 13.7943C13.6353 13.5124 13.6447 13.045 13.3617 12.7518ZM5.67407 1.47456C7.98662 1.47456 9.86795 3.34873 9.86795 5.65249C9.86795 7.95625 7.98662 9.83043 5.67407 9.83043C3.36152 9.83043 1.48019 7.95625 1.48019 5.65249C1.48019 3.34873 3.36152 1.47456 5.67407 1.47456Z" fill="#FB4B54"/>
              </svg>
            </a>
          </object>
          <div class="product-card__img">
            <img src="img/b1.jpg" alt="">
          </div>
          <div class="product-card__content">
            <div class="product-card__name">
              B1
            </div>
            <div class="product-card__info">
              <div class="product-price">
                <div class="product-price__old">
                  15000 $
                </div>
                <div class="product-price__actual">
                  <div class="product-price__new">
                    4000 $
                  </div>
                  <div class="product-price__quantity">
                    за 10 шт
                  </div>
                </div>
              </div>
              <object>
                <a data-toggle="modal" data-target="#button-tovar-v-korzine" class="in-cart">
                  В корзину
                </a>
              </object>
            </div>
          </div>
        </a>
      </div>
    </div>
  </div>
  <?php include 'footer.php' ?>
